...
public class Skydome implements Disposable {
    ...
    public Skydome() {
        ...
        skydomeModelInstance = new ModelInstance(Assets.skydomeModel);
        skydomeModelInstance.getMaterial("sky").set(TextureAttribute.createDiffuse(atmosphereRegion));
    }

    public ModelInstance getModelInstance() {
        return skydomeModelInstance;
    }
    ...
    public void update(float delta, Camera camera) {
        skydomeModelInstance.transform.setTranslation(camera.position.x, camera.position.y - 30f, camera.position.z);
    ...
    // ------------------------------------------------------------------------
    // Atmospheric Scattering Implementation Details
    // ------------------------------------------------------------------------

    private float exp(float fValue) {
        return (float) Math.exp(fValue);
    }

    private float tan(float fValue) {
        return (float) Math.tan(fValue);
    }

    private float sqr(float fValue) {
    ...
    private void renderAtmosphereToTexture() {
        atmosphereFBO.begin();
        {
            ...
            atmosphereShader.setUniform3fv("windowDimensions",
                                           new float[] { Gdx.graphics.getWidth(), Gdx.graphics.getHeight(), 0.f },
                                           0,
                                           3);
            ...
            atmosphereShader.setUniform3fv("colourCorrection",
                                           new float[] { exposure, overcast, gammaCorrection },
                                           0,
                                           3);
